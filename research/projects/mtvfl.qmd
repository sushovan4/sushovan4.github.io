...
<header class="masthead page">
  <div class="kicker">Research &middot; Plate IX</div>
...
- ***Tracking-free topological activity in gas&ndash;liquid pipe flow: calibration, precision, and what the rate measures***&mdash;an Euler-characteristic activity channel read from eleven hours of video across two pipe diameters, calibrated against high-speed conductance probes before any physics is read from it. To first order it measures how strongly and how quickly the count of large gas structures fluctuates, not individual interfacial events; so read, activity falls from slug to annular flow and couples to the arrival rate of large structures. In preparation, for *Experiments in Fluids*.
- ***What evaluation conventions are worth in image-based flow-regime identification***&mdash;what preprocessing, evaluation conventions and manual steps actually cost, measured on a public drilling-annulus benchmark and on the flow-loop archive. Choices usually reported in a single sentence move conclusions by more than most of the physical effects under study. In preparation, for *Data-Centric Engineering*.
